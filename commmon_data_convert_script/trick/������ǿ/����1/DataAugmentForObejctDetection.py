...
def show_pic(img, bboxes=None):
    ...
    输入:
        ...
        names:每个box对应的名称
    '''
    cv2.imwrite('./1.jpg', img) #搞一个临时文件，防止展示原图的时候img已经带了bbox框，没法再用于后面数据增强然后展示
...
if __name__ == '__main__':
    ...
    # 如果没有保存数据增强图片的文件夹，则创建
    if not os.path.exists(img_path + 'augimg'):
        os.makedirs(img_path + 'augimg')
    ...
    for i in range(len(img_list)):
        ...
        for j in range(len(data_list)):
            if data_list[j]['name'] == img_list[i]:  # 匹配
                # 1、找到对应的坐标
                coord = []
                coord.append(data_list[j]['bbox'][0])
                coord.append(data_list[j]['bbox'][1])
                coord.append(data_list[j]['bbox'][2])
                coord.append(data_list[j]['bbox'][3])
                coords.append(coord)
        ...
        # 数据增强后的图
        auged_img, auged_bboxes = dataAug.dataAugment(img, coords)
        #show_pic(auged_img, auged_bboxes)  # 显示数据增强后的图

        # 保存增强后的图片
        if image_name.endswith('.jpg'):
            cv2.imwrite(img_path + 'augimg/' + image_name.split('.')[0] + '_' + str(j) + '.jpg', auged_img)
