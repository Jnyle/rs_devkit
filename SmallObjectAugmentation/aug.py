...
def convert_all_boxes(shape, anno_infos, yolo_label_txt_dir):
    ...
    for anno_info in anno_infos:
        x1, y1, x2, y2 = anno_info[:4]
        bb = (float(x1), float(y1), float(x2), float(y1), float(x2), float(y2), float(x1), float(y2), 'swimming-pool', '0')
...
def suo_fang(image, area_max=5000, area_min=1000):
    # 改变图片大小
...
def copysmallobjects2(image_dir, label_dir, save_base_dir, small_img_dir, i):
    image = cv2.imread(image_dir)
    labels = read_label_txt(label_dir)
    all_boxes = []
    rescale_labels = []
    ...
    for small_img_dirs in small_img_dir:
        ...
        roi = suo_fang(image_bbox, area_max=2300, area_min=500)
    ...
    save_dir = join(save_base_dir, dir_name)
    check_dir(save_dir)
    yolo_txt_dir = join(save_dir, basename(image_dir.replace('.png', '_{}.txt'.format(i))))
    cv2.imwrite(join(save_dir, basename(image_dir).replace('.png', '_{}.png'.format(i))), image)
    convert_all_boxes(image.shape, all_boxes, yolo_txt_dir)
