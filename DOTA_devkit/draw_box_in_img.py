...
STANDARD_COLORS2 = [
    'Blue', 'Cyan', 'Yellow', 'Red', 'Magenta', 'Orange', 'Pink', 'SpringGreen', 'Firebrick',
    'Indigo', 'MediumSlateBlue', 'SaddleBrown', 'Goldenrod', 'LimeGreen', 'Purple',
    'Navy', 'PaleVioletRed', 'Salmon', 'Goldenrod'
]
...
def draw_boxes_with_label_and_scores(img_array, boxes, scores, labels,  mode=0):
    ...
    for box, a_score, a_label in zip(boxes, scores, labels):
        ...
        draw_a_rectangel_in_img(draw_obj, box, a_score, a_label, color=STANDARD_COLORS2[ODAI_LABEL_MAP[a_label] - 1], width=2, mode=mode)
...
def draw_a_rectangel_in_img(draw_obj, box, score, label, color, width, mode):
    if mode == 0:
        ...
        draw_obj.line(xy=[top_right, bottom_right],
                      fill=color,
                      width=width)
        txt = label + ':' + str(round(score, 2))
        draw_obj.rectangle((x1, y1, (x1 + len(txt) * 5 + 20), y1 - 10), fill=color)
        draw_obj.text(xy=(x1, y1 - 10),
                      text=txt,
                      fill='White',
                      font=FONT)
    else:
        ...
        draw_obj.line(xy=[(box[6], box[7]), (box[0], box[1])],
                      ...
                      width=width)
        txt = label + ':' + str(round(score, 2))
        draw_obj.rectangle((box[0], box[1], (box[0] + len(txt) * 5 + 20), box[1] - 10), fill=color)
        draw_obj.text(xy=(box[0], box[1] - 10),
                      text=txt,
                      fill='White',
                      font=FONT)
...
if __name__ == '__main__':
    ...
    imm = draw_boxes_with_label_and_scores(img_array, boxes2, labels, scores, mode=0)

    cv2.imshow("te", imm)
    cv2.imwrite('3.png', imm)
    cv2.waitKey(0)