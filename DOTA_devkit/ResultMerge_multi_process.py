...
def py_cpu_nms(dets, thresh):
    ...
    while order.size > 0:
        ...
        h1 = y2[i] - y1[i]
        x_center = w1 / 2
        x_other = (x2[order[1:]] - x1[order[1:]]) / 2
        y_center = h1 / 2
        y_other = (y2[order[1:]] - y1[order[1:]]) / 2
        distance = np.sqrt((x_center - x_other) ** 2 + (y_center - y_other) ** 2)
        # inds = np.where(ovr <= thresh)[0]
        inds = np.where((ovr <= thresh) & (ovr_iof <= 0.8) & (distance <= min(w1, h1)))[0]
        order = order[inds + 1]
...
if __name__ == '__main__':
    mergebypoly(r'/mnt/lustre/yanhongchang/project/one-rpn/mmdetection/demo/work_dirs/out_img/faster_x101/crop512/txt',
                r'/mnt/lustre/yanhongchang/project/one-rpn/mmdetection/demo/work_dirs/out_img/faster_x101/crop512/txt2')
    # mergebyrec()